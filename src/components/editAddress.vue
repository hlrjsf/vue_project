  ...
  <div class="Main">
    <div class="secondary_head">
		<div class="go_back" @click="$router.go(-1);">
      	...
      	<div class="header_title">{{editAddress}}</div>
    </div>
    <div class="content_mian">
      <form action="" method="post" name="address_edit" id="address_edit">
  			<div class="address_list">
  				<div class="user_name">
  					<span>收 件 人 ：</span>
            <input type="text" v-model="userName" @focus="foucesBorder($event)" @blur="blurBorder($event)">
  				</div>
          <div class="user_phone">
            <span>手机号码 ：</span>
            <input type="text" v-model="userPhone" @focus="foucesBorder($event)" @blur="blurBorder($event)">
          </div>
  				<div class="user_area" @click="choseAdd">
  					<span>所在地区 ：</span>
            <input type="text" v-model="userArea">
  				</div>
  				<div class="detail_address">
  					<span>详细地址 ：</span>
            <input type="text" v-model="detailAddress" @focus="foucesBorder($event)" @blur="blurBorder($event)">
  				</div>
  			</div>
      </form>
      <input type="submit" value="确定" class="sure_edit b_bg" id="confirm"/>
    </div>
    <div class="address_slecte">
      <section class="showChose" v-show="showChose">
        <section class="address">
          <section class="title">
            <h4>居住地址</h4>
            <span class="b_color" @click="closeAdd()">×</span>
          </section>
        ...
        <div class="sure_cancle_btn">
          <div class="sure_btn b_bg" @click="sureAddress()">确认</div>
          <div class="cancle_btn" @click="closeAdd()">取消</div>
        </div>
      </section>
    </div>
  </div>
...
export default {
  ...
  methods: {
    foucesBorder: function(e){
      var el = e.srcElement;
      el.parentElement.className = 'active';
    },
    blurBorder: function(e){
      var el = e.srcElement;
      el.parentElement.className = 'unactive';
    },
    choseAdd: function() {
      this.showChose = true;
    },
    ...
    sureAddress: function(){
      var province = this.info[this.province-1].name;
      var city = this.City;
      var district = this.District;
      // 点击之后关闭弹层
      this.showChose = false;
      this.userArea = province + ' ' + city + ' ' + district;
    }
  }
}
</script>
...
.address .title .active{
  color:#fea31e;
  border-bottom:0.02rem solid #fea31e;
}
.address ul .active{
  color:#fea31e;
}
